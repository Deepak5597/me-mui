        ...
        <Box sx={{ bgcolor: "white", height: "87vh", width: "100%", p: 2, display: "flex", flexDirection: "column" }}>
            ...
            <Box component="form" sx={{ display: "flex", flexWrap: "wrap", mb: 1 }} noValidate autoComplete="off">
...

                {
                    ...
                    <FormControl sx={{ m: 1, mr: 3, width: "max-content" }}>
                        <TextField disabled id="outlined-required" label="Billing Type" value={saleForm?.billingType} />
                    </FormControl>
                }
                {
                    saleForm?.partyDetails?.billingLocation?.billingContactNumber &&
                    <FormControl sx={{ m: 1, mr: 3 }}>
                        <TextField disabled id="outlined-required" label="Contact Number" value={saleForm?.partyDetails?.billingLocation?.billingContactNumber} />
                    </FormControl>
                }
                {
                    saleForm?.partyDetails?.billingLocation?.billingAddress &&
                    <FormControl sx={{ m: 1, mr: 3, flex: 1 }}>
                        <TextField disabled id="outlined-required" label="Address" value={saleForm?.partyDetails?.billingLocation?.billingAddress} />
                    </FormControl>
                }
            </Box>
            <Divider />
